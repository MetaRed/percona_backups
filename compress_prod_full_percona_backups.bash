...
if [ ! $? -eq 0 ]; then
  echo "Unable to change ownership of the current backup $LOCAL_BACKUP_DIR/${SERVER_NAME}-$CURRENT_BACKUP_DIR" |mail -s "${0}: failed on $SERVER_NAME" $EMAIL
  exit 1
fi
...
if [ ! $? -eq 0 ]; then
  echo "Unable to find the current sql dump file ${CURRENT_SQLDUMP_FILE}" |mail -s "${0}: failed on $SERVER_NAME" $EMAIL
fi

# compress the current structure backup
gzip ${CURRENT_SQLDUMP_FILE}
if [ ! $? -eq 0 ]; then
  echo "Unable to gzip the current sql dump file ${CURRENT_SQLDUMP_FILE}" |mail -s "${0}: failed on $SERVER_NAME" $EMAIL
  exit 1
fi

# change ownership for encryption
chown gpg_user:gpg_user ${CURRENT_SQLDUMP_FILE}.gz
mv ${CURRENT_SQLDUMP_FILE}.gz ${SERVER_NAME}-${CURRENT_SQLDUMP_FILE}.gz
